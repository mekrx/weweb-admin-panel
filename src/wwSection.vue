  ...
  <div class="admin-panel" :style="cssVars">
    <div v-if="!supabase" class="setup-notice">
      <div class="setup-card">
        ...
        <div class="setup-text">Oczekiwanie na połączenie z Supabase. Upewnij się, że plugin Supabase jest podłączony w WeWeb i jesteś zalogowany. Alternatywnie podaj URL i Anon Key w ustawieniach komponentu.</div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.initSupabase();
    // Retry after short delay in case WeWeb plugin loads after our component
    setTimeout(() => {
  ...
  methods: {
    async initSupabase() {
      try {
        // Method 1: Use WeWeb's existing authenticated Supabase instance (preferred)
        if (typeof wwLib !== 'undefined') {
          const pluginInstance = wwLib?.wwPlugins?.supabase?.instance 
            || wwLib?.wwPlugins?.supabaseAuth?.publicInstance;
          if (pluginInstance) {
            this.supabase = pluginInstance;
            await this.loadInitialData();
            return;
          }
        }
        // Method 2: Fallback to manual connection with URL + Key (for testing)
